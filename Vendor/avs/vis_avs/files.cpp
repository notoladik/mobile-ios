...
#if (defined(__linux__) || defined(__APPLE__)) && !defined(FORCE_CPP_FILESYSTEM_API)
#include <ftw.h>
#include <glob.h>
...
// "thread_local" makes a new instance of this variable for each thread.
// On iOS / older Clang targets, thread_local is not supported, so use static.
#if defined(__APPLE__) || !defined(__cpp_thread_local)
static ftw_data_t ftw_data;
#else
