                               ...
                               bool smooth_transition)
{
    ...
    {
        LOG_ERROR(std::string("[projectM] LoadPresetFile exception: ") + e.what());
    }
    catch (...)
    {
        LOG_ERROR("[projectM] Unknown exception in LoadPresetFile");
    }
}
                               ...
                               bool smooth_transition)
{
    ...
    {
        LOG_ERROR(std::string("[projectM] LoadPresetData exception: ") + e.what());
    }
    catch (...)
    {
        LOG_ERROR("[projectM] Unknown exception in LoadPresetData");
    }
}
...
{
    projectm_opengl_render_frame_fbo(instance, 0);
}
...
void projectm_opengl_render_frame_fbo(projectm_handle instance, uint32_t framebuffer_object_id)
{
    ...
    {
        LOG_ERROR(std::string("[projectM] RenderFrame exception: ") + e.what());
    }
    catch (...)
    {
        LOG_ERROR("[projectM] Unknown exception in RenderFrame");
    }
}
...
void projectm_get_window_size(projectm_handle instance, size_t* width, size_t* height)
{
    ...
    if (projectMInstance)
    {
        if (width) *width = static_cast<size_t>(projectMInstance->WindowWidth());
        if (height) *height = static_cast<size_t>(projectMInstance->WindowHeight());
    }
}
