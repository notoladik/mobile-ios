...
void Framebuffer::SetAttachment(int framebufferIndex, int attachmentIndex, const std::shared_ptr<TextureAttachment>& attachment)
{
    ...
    if (m_drawFramebuffer >= 0 && m_drawFramebuffer < static_cast<int>(m_framebufferIds.size()))
    {
        glBindFramebuffer(GL_FRAMEBUFFER, m_framebufferIds.at(m_drawFramebuffer));
    }
    else
    {
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
