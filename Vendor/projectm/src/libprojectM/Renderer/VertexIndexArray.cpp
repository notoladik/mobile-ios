...
void VertexIndexArray::Update()
{
    ...
    shortIndices.reserve(m_indices.size());
    for (auto idx : m_indices) {
        shortIndices.push_back(static_cast<uint16_t>(idx));
    }

    if (m_veabSize == m_indices.size())
    {
        glBufferSubData(GL_ELEMENT_ARRAY_BUFFER, 0, static_cast<GLsizei>(sizeof(uint16_t) * shortIndices.size()), shortIndices.data());
    }
    else
    {
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, static_cast<GLsizei>(sizeof(uint16_t) * shortIndices.size()), shortIndices.data(), VertexBufferUsageToGL(m_vboUsage));
