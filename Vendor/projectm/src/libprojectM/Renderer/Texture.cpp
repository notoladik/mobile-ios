...
void Texture::Bind(GLint slot, const Sampler::Ptr& sampler) const
{
    ...
    if (sampler)
    {
        glTexParameteri(m_target, GL_TEXTURE_WRAP_S, sampler->WrapMode());
        glTexParameteri(m_target, GL_TEXTURE_WRAP_T, sampler->WrapMode());
        glTexParameteri(m_target, GL_TEXTURE_MIN_FILTER, sampler->FilterMode());
        glTexParameteri(m_target, GL_TEXTURE_MAG_FILTER, sampler->FilterMode());
    }
    else
    {
